...
export default function ManageUserCategories() {
  ...
  const [successMessageAdd, setSuccessMessageAdd] = useState(""); // New state for success message
  ...
  const handleAddUserToCategory = async (e) => {
    ...
    try {
      ...
      setErrorMessageAdd(""); // Clear error message
      setValidSelectionAdd(true); // Reset validation
      await fetchData(); // Refresh the user list
      setSelectedUserAdd(""); // Reset form
      setSelectedCategoryAdd(""); // Reset form
    } catch (error) {
  ...
  const userCategoryRelationships = users
    .filter(
      (user) => Array.isArray(user.categories) && user.categories.length >= 2
    )
    .flatMap((user) =>
      ...
      <div className="bg-white rounded-xl shadow-md p-6">
        ...
        <div className="mb-8">
          ...
          <form onSubmit={handleAddUserToCategory} className="flex gap-4">
            ...
            <button
              type="submit"
              className="px-4 py-2 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 focus:ring-4 focus:ring-blue-300 transition duration-200"
            >
              Add
            </button>
          </form>
          {!validSelectionAdd && (
            <div className="text-red-500 text-sm mt-2">{errorMessageAdd}</div>
          )}
          {successMessageAdd && (
        ...
        <div className="mb-8">
          ...
          <form onSubmit={handleRemoveUserFromCategory} className="flex gap-4">
            ...
            <button
              type="submit"
              className="px-4 py-2 bg-red-600 text-white font-medium rounded-lg hover:bg-red-700 focus:ring-4 focus:ring-red-300 transition duration-200"
            >
              Remove
            </button>
          </form>
        ...
        {userCategoryRelationships.length === 0 ? (
          <div className="text-center py-6">
            ...
            <p className="mt-2 text-gray-600">
              No users with two or more category relationships found.
            </p>
          </div>
        ) : (
            ...
            <table className="w-full bg-white rounded-lg">
              ...
              <tbody>
                {userCategoryRelationships.map((relationship) => (
                  ...
                  >
                    <td className="px-6 py-4">{relationship.user_name}</td>
                    <td className="px-6 py-4">{relationship.category_name}</td>
                  </tr>
                ))}
              </tbody>
            </table>
